...
function Events(_a) {
  ...
  var _f = useState(''),
    ...
    setEndTime = _f[1];
  var _g = useState([]),
    selectedFamilyMembers = _g[0],
    setSelectedFamilyMembers = _g[1];
  var _h = useState(null),
    errorMessage = _h[0],
    setErrorMessage = _h[1];
  var _j = useState(false),
    isAddingEvent = _j[0],
    setIsAddingEvent = _j[1]; // Lägg till event-form visas ej från början
  //const [events, setEvents] = useState<Event[]>([]);
  var _k = useState([]),
    familyMembers = _k[0],
    setFamilyMembers = _k[1];
  ...
  function toggleFamilyMember(id) {
    if (selectedFamilyMembers.includes(id)) {
      setSelectedFamilyMembers(
        selectedFamilyMembers.filter(function (fm) {
          return fm !== id;
        }),
      );
    } else {
      setSelectedFamilyMembers(
        __spreadArray(
          __spreadArray([], selectedFamilyMembers, true),
          [id],
          false,
        ),
      );
        ...
        switch (_a.label) {
          ...
          case 3:
            ...
            /*if (Array.isArray(data)) {
                          setEvents(data);*/
            if (Array.isArray(data)) {
              mappedEvents = data.map(function (ev) {
                return {
                  id: ev.id,
                  title: ev.title,
  ...
  return _jsxs(EventsFormContainer, {
    children: [
      _jsx(Title, { children: 'Aktiviteter' }),
      _jsxs(FormContainer, {
        children: [
          !isAddingEvent &&
            _jsx(SubmitButton, {
              onClick: /*För att öppna formuläret */ function () {
                return setIsAddingEvent(true);
              },
              children: 'L\u00E4gg till nytt event',
            }),
          isAddingEvent &&
            _jsxs(StyledForm, {
              ...
              children: [
                ...
                _jsxs(FormGroup, {
                  children: [
                    _jsx('label', { children: 'Beskrivning:' }),
                    _jsx('textarea', {
                      value: description,
                      onChange: function (event) {
                ...
                _jsxs(FormGroup, {
                  children: [
                    _jsx('label', { children: 'Familjemedlemmar (minst en):' }),
                    familyMembers.length === 0 &&
                      _jsx(NoFamilyMessage, {
                        children: 'Inga familjemedlemmar hittades.',
                      }),
                        ...
                        CheckboxLabel,
                        {
                          children: [
                            _jsx('input', {
                              ...
                              onChange: function () {
                                return toggleFamilyMember(fm.id);
                              },
                            }),
                            fm.name,
                          ],
                ...
                errorMessage && _jsx(ErrorMessage, { children: errorMessage }),
                _jsx(SubmitButton, { type: 'submit', children: 'Skapa Event' }),
              ],
            }),
        ],
      }),
      Array.isArray(events) && events.length > 0
              ...
              return _jsxs(
                EventItem,
                {
                  children: [
                    ...
                    _jsx('strong', { children: event.title }),
                    _jsx('br', {}),
                    /* Skapa ett Date-objekt med newDate(), omvandla till ISO-string .split('T' delar upp strängen i två delar,
                        och [0] tar första delen av arrayen som är datumet*/ new Date(
                      event.event_date,
                    )
                      .toISOString()
                      .split('T')[0],
                    _jsx('br', {}), // Visa start- och sluttid utan sekunder genom att ta bort 5 första tecknena
                    event.start_time && event.end_time
                      ? ''
                          .concat(event.start_time.slice(0, 5), ' - ')
                          .concat(event.end_time.slice(0, 5))
                      : '',
                    _jsx('br', {}), // Lägger till description om det finns
                    event.description &&
                      _jsx('em', { children: event.description }),
                    _jsx('br', {}),
                    ...
                    event.family_member_ids.length === 0
                      ...
                      : event.family_member_ids
                          .map(function (id) {
                            var _a;
                            return (_a = familyMembers.find(function (fm) {
                              return fm.id === id;
                            })) === null || _a === void 0
                              ? void 0
                              : _a.name;
        ...
        : _jsx('p', { children: 'Inga events skapade \u00E4n.' }),
    ],
  });
}
export default Events;
// styling
var EventsFormContainer = styled.div(
  templateObject_1 ||
