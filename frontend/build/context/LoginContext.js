...
export var LoginProvider = function (_a) {
    ...
    var _b = useState(localStorage.getItem('token')), token = _b[0], setToken = _b[1];
    var _c = useState(localStorage.getItem('userEmail')), userEmail = _c[0], setUserEmail = _c[1];
    var _d = useState(localStorage.getItem('userName')), userName = _d[0], setUserName = _d[1];
    var _e = useState(localStorage.getItem('userId') ? Number(localStorage.getItem('userId')) : null), userId = _e[0], setUserId = _e[1];
    // Login-funktion 
    ...
    return (_jsx(LoginContext.Provider, { value: { token: token, userEmail: userEmail, userName: userName, userId: userId, login: login, logout: logout, isLoggedIn: isLoggedIn }, children: children }));
};
