...
export var LoginProvider = function (_a) {
    ...
    // Login-funktion 
    var login = function (token, email, name) {
        setToken(token);
        setUserEmail(email);
        setUserName(name);
    ...
    return (_jsx(LoginContext.Provider, { value: { token: token, userEmail: userEmail, userName: userName, login: login, logout: logout, isLoggedIn: isLoggedIn }, children: children }));
};
